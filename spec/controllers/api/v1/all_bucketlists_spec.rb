...
RSpec.describe "list all the bucketlists", type: :request do
  ...
  context "with valid search querry" do
    before(:all) do
      bucketlist = create(:bucket_list, name: "Late Thirties", user: @user)
      bucketlist2 = create(:bucket_list, name: "Early Thirties", user: @user)
      bucketlist3 = create(:bucket_list, name: "Mid Twenties", user: @user)
      headers = {
        "HTTP_AUTHORIZATION" => @token,
        "Content-Type" => "application/json",
        "HTTP_ACCEPT" => "application/vnd.bucketlist.v1"
      }
      get "/bucketlists/?query=Thirties", {}, headers
    end
    ...
    it "return bucketlist belonging to current user" do
      bucket_list_name = json.first.map{|hsh| hsh[:name] }
  ...
  context "with invalid search querry" do
    before(:all) do
      ...
      get "/bucketlists/?query=party",{}, headers
    end
